...
namespace Mapbox.Unity.Utilities
{
	...
	public static class Conversions
	{
		...
		public static Vector2d StringToLatLon(string s)
		{
			...
			{
				throw new ArgumentException($"Wrong number of arguments in StringToLatLon, {s}");
			}


            if (!double.TryParse(latLonSplit[0], NumberStyles.Any, NumberFormatInfo.InvariantInfo, out double latitude))
            {
                throw new Exception(string.Format("Could not convert latitude to double: {0}", latLonSplit[0]));
            }

            if (!double.TryParse(latLonSplit[1], NumberStyles.Any, NumberFormatInfo.InvariantInfo, out double longitude))
			{
				throw new Exception(string.Format("Could not convert longitude to double: {0}", latLonSplit[0]));
			}

            //Debug.Log($"Converted: {s}, to {latitude}, {longitude}");
		...
		private static Vector2d PixelsToMeters(Vector2d p, int zoom)
		{
            ...
            {
                x = (p.x * res - OriginShift),
                y = -(p.y * res - OriginShift)
            };
            return met;
		}
